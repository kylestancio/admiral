...
export default function EditUserForm({user}:{user:UserWithoutPassword}) {
  ...
  const {
    ...
    defaultValues: {
      ...
      phone: user.phone || '',
      email: user.email || '',
      birthday: user.birthday.toISOString().substring(0, 10),
      employmentDate: user.employmentDate.toISOString().substring(0, 10),
      roles: user.roles
    }
  })
  ...
  const submitHandler: SubmitHandler<z.infer<typeof schema>> = async (data) => {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_URL}/api/user/update`, {
      method: 'PATCH',
      body: formdata
    })
    .then(res=>{
